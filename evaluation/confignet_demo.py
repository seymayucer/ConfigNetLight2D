...
from confignet import ConfigNet, LatentGAN
...
def parse_args(args):
    ...
    parser.add_argument(
        "--image_path",
        help="Path to either a directory of images or an individual image",
        default=None,
    )
    parser.add_argument(
        "--confignet_model_path", help="Path to confignet model", default=None
    )
    parser.add_argument(
        "--latent_gan_model_path", help="Path to latentgan model", default=None
    )
    parser.add_argument(
        "--resolution", type=int, help="Path to ConfigNetModel", default=256
    )
    parser.add_argument(
        "--n_rows", type=int, help="Number of rows in image matrix", default=2
    )
    parser.add_argument(
        "--n_cols", type=int, help="Number of columns in image matrix", default=3
    )
    parser.add_argument(
        "--test_mode",
        help="If specified only one frame will be generated, this is used in unit tests",
        action="store_true",
        default=False,
    )
...
def process_images(image_path: str, resolution: int) -> List[np.ndarray]:
    ...
    #     img = FaceImageNormalizer.normalize_individual_image(
    ...
    #         image_path,
    #         pre_normalize=True,
    #         output_image_shape=(resolution, resolution),
...
def get_new_embeddings(
    args, input_images, latentgan_model: LatentGAN, confignet_model: ConfigNet
):
    ...
    if input_images is None:
        ...
        orig_images = confignet_model.generate_images(embeddings)
    else:
        ...
        orig_images = np.array([input_images[x] for x in range(n_samples)])
        embeddings = confignet_model.encode_images(orig_images)

    return embeddings, orig_images


def get_embedding_with_new_attribute_value(
    parameter_name: str, latents: np.ndarray, confignet_model: ConfigNet
) -> np.ndarray:
...
def print_intro():
    print("")
    print(
        "This demo demonstrates the cabalities of ConfigNet to modify existing face images."
    )
    print("If no argument is specified the face will be ampled from a LatentGAN.")
    print(
        "If you want to modify specific images pass a path to an individual image or a directory as argument."
    )
    print("")
...
def print_instructions():
    ...
    print(
        "Make sure that the window with the face renders is in focus when pressing the keys."
    )
    print("")
    print(
        "space - sample another set of images from input directory or latent gan (if input directory not specified"
    )
    ...
    print(
        "B - fine-tune the generator on the chosen image (one-shot learning), works only if a single image is passed as input"
    )
...
def run(args):
    ...
    current_embedding_unmodified, orig_images = get_new_embeddings(
    ...
    while not basic_ui.exit:
        ...
        #     current_renderer_input, basic_ui.eye_rotation_offset, confignet_model
        # )

        generated_imgs = confignet_model.generate_images(current_renderer_input)

        white_strip = np.full(
            (generated_imgs.shape[0], generated_imgs.shape[1], 20, 3), 255, np.uint8
        )
        ...
        if key == ord(" ") or args.test_mode:
            (
                current_embedding_unmodified,
                orig_images,
            ) = get_new_embeddings(args, input_images, latentgan_model, confignet_model)
            basic_ui.set_next_embeddings(current_embedding_unmodified)
        ...
        if key == ord("x") or args.test_mode:
            ...
            new_embedding_value = get_embedding_with_new_attribute_value(
                ...
                basic_ui.get_current_frame_embeddings(),
                confignet_model,
            )
        ...
        if key == ord("b") or args.test_mode:
            if input_images is None or len(input_images) != 1:
                ...
                print(
                    "For one-shot learning to work you need to specify a single input image path,taking the first image from the input directory"
                )

            if args.test_mode:
                n_fine_tuning_iters = 1
            else:
                n_fine_tuning_iters = 1000
            print(
            ...
            (current_embedding_unmodified,) = confignet_model.fine_tune_on_img(
                input_images[0], n_fine_tuning_iters, img_output_dir="."
            )
            basic_ui.set_next_embeddings(current_embedding_unmodified)
