...
class BasicUI:
    def __init__(self, confignet_model):
        ...
        self.facemodel_param_names = list(
            self.confignet_model.config["facemodel_inputs"].keys()
        )
        ...
        # hdri_turntable_file_path = os.path.join(
        #     os.path.dirname(__file__), "..", "assets", "hdri_turntable_embeddings.npy"
        # )
    ...
    def get_current_frame_embeddings(self):
        ...
        frame_embedding = (
            self.render_input_interp_0 * (1 - self.interpolation_coef)
            + self.render_input_interp_1 * self.interpolation_coef
        )

        # if self.sweeping_hdri:
        #     hdri_params = self.hdri_turntable_embeddings[
        #         self.current_hdri_embedding_frame
        #     ]
        #     frame_embedding = self.confignet_model.set_facemodel_param_in_latents(
        #         frame_embedding, "hdri_embedding", hdri_params
        #     )
        #     self.current_hdri_embedding_frame = (
        #         self.current_hdri_embedding_frame + 1
        #     ) % len(self.hdri_turntable_embeddings)
    ...
    def print_instructions():
        ...
        print(
            "N - play a pre-set illumination sequence that rotates the light source around, pressing again ends the sequence"
        )
    ...
    def drive_ui(self, key: str, test_mode=False):
        ...
        if key == ord("z") or test_mode:
            self.controlled_param_idx = (self.controlled_param_idx - 1) % len(
                self.facemodel_param_names
            )
            print(
                "Currently controlled face model parameter:",
                self.facemodel_param_names[self.controlled_param_idx],
            )
