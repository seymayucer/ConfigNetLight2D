...
def parse_args(args):
    ...
    parser.add_argument(
        "--output_dir",
        help="Directory where training output will be stored",
        default=None,
        required=True,
    )
    parser.add_argument(
        "--num_mlp_layers",
        type=int,
        help="Number of discriminator and generator layers",
        default=DEFAULT_CONFIG["num_mlp_layers"],
    )
    parser.add_argument(
        "--hidden_layer_size_multiplier",
        type=float,
        help="Multiplier for number of units in hidden layers",
        default=DEFAULT_CONFIG["hidden_layer_size_multiplier"],
    )
    parser.add_argument(
        "--latent_distribution_type",
        help="Type of distribution, normal or uniform",
        default=DEFAULT_CONFIG["latent_distribution_type"],
    )
    parser.add_argument(
        "--batch_size",
        type=int,
    ...
    parser.add_argument(
        "--n_training_steps",
        type=int,
        help="Number of training steps that will be run",
        default=200000,
    ),
    parser.add_argument(
    ...
    parser.add_argument(
        "--log_dir",
        help="Path to which the tensorboard logs will be written, defaults to output_dir",
        default=None,
    )
