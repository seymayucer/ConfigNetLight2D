...
class ControllabilityMetrics:
    def __init__(self, confinet_model, attribute_classifier, per_image_tuning_iters=0):
        ...
        if isinstance(attribute_classifier, CelebaAttributeClassifier):
            ...
            self.attribute_classifier = CelebaAttributeClassifier.load(
                attribute_classifier
            )

        self.per_image_tuning_iters = per_image_tuning_iters
        if confinet_model is not None:
            self.facemodel_param_names = list(
                self.confinet_model.config["facemodel_inputs"].keys()
            )
    ...
    def get_images_for_controllable_attribute(
        ...
        latent_vector_with_attribute_set = self.confinet_model.synthetic_encoder(
            facemodel_params
        )

        modified_param_idx = self.facemodel_param_names.index(
            attribute_config.facemodel_param_name
        )
        facemodel_param_dims = list(
            self.confinet_model.config["facemodel_inputs"].values()
        )
        start_idx = int(
            np.sum([x[1] for x in facemodel_param_dims[:modified_param_idx]])
        )
        end_idx = start_idx + facemodel_param_dims[modified_param_idx][1]

        modified_latent_vectors = np.copy(latent_vectors)
        modified_latent_vectors[
            :, start_idx:end_idx
    ...
    def generate_images_for_metric(self, input_images):
        if self.per_image_tuning_iters > 0:
            ...
            for key in images_without_attributes.keys():
                images_without_attributes[key] = np.array(
    ...
    def get_metrics_for_attribute_pairs(
        ...
        changing_attribute_names = attribute_config.ignored_attributes + [
            attribute_config.driven_attribute
        ]
        # These attributes are expected to be constant
        constant_attribute_idxs = [
            i
            for i, attribute_name in enumerate(attribute_names)
            if attribute_name not in changing_attribute_names
        ]

        mean_value_of_attribute_after_setting = np.mean(
            set_attributes[:, driven_attribute_idx]
        )
        mean_value_of_attribute_after_setting_other = np.mean(
            not_set_attributes[:, driven_attribute_idx]
        )
        ...
        attribute_values_for_correlation = np.hstack(
            (np.ones(n_samples), np.zeros(n_samples))
        )
        predicted_values_for_correlation = np.hstack(
            (
                set_attributes[:, driven_attribute_idx],
                not_set_attributes[:, driven_attribute_idx],
            )
        ...
        corr_coef = np.corrcoef(
            np.vstack(
                (attribute_values_for_correlation, predicted_values_for_correlation)
            )
        )

        mad_of_const_attributes = np.mean(
            np.abs(
                set_attributes[:, constant_attribute_idxs]
                - not_set_attributes[:, constant_attribute_idxs]
            ),
            axis=0,
        )
        ...
        return (
            float(mean_value_of_attribute_after_setting),
            float(mean_value_of_attribute_after_setting_other),
            float(mad_of_const_attributes),
            float(corr_coef[0, 1]),
        )

    def get_metrics_for_attribute_config(
        self, attribute_config, images_with_attribute, images_without_attribute
    ):
        set_attributes = self.attribute_classifier.predict_attributes(
            images_with_attribute
        )
        not_set_attributes = self.attribute_classifier.predict_attributes(
            images_without_attribute
        )
        metrics = self.get_metrics_for_attribute_pairs(
            set_attributes, not_set_attributes, attribute_config
        )
    ...
    def get_metrics(self, input_images, img_output_dir=None):
        (
            ...
            images_with_attributes,
            images_without_attributes,
        ) = self.generate_images_for_metric(input_images)
        if img_output_dir is not None:
            ...
            for i in range(len(input_images)):
                ...
                cv2.imwrite(
                    os.path.join(img_output_dir, "raw_img_%04d.png" % i),
                    raw_decoded_images[i],
                )
                for config_name, _ in ControllabilityMetricConfigs.all_configs():
        ...
        return self.get_metrics_from_attribute_images(
            images_with_attributes, images_without_attributes
        )

    def get_metrics_from_attribute_images(
        self, images_with_attributes, images_without_attributes
    ):
        metrics = {}
        for config_name, attribute_config in ControllabilityMetricConfigs.all_configs():
            attribute_metrics = self.get_metrics_for_attribute_config(
                attribute_config,
                images_with_attributes[config_name],
                images_without_attributes[config_name],
            )
        ...
        metrics["contr_attribute_means"] = tuple(
            np.mean(list(metrics.values()), axis=0)
        )
        # Weights chosen basen on perceived importance
        metrics["controllability"] = 10 * metrics["contr_attribute_means"][2] + (
            1 - metrics["contr_attribute_means"][0]
        )
    ...
    def update_and_log_metrics(
                ...
                for key, value in new_metrics.items():
                    if isinstance(value, tuple):
                        ...
                        tf.summary.scalar(
                            prefix + key + "_post",
                            value[0],
                            step=metrics_dict["training_step_number"][-1],
                        )
                        tf.summary.scalar(
                            prefix + key + "_pre",
                            value[1],
        ...
        metrics_dict_contr_only = dict(
            [(key, metrics_dict[key]) for key in new_metrics.keys()]
        )
...
class InceptionMetrics:
    def __init__(self, confignet_config, dataset, n_samples_for_metrics=1000):
        ...
        self.inception_feature_extractor = InceptionFeatureExtractor(
            confignet_config["output_shape"]
        )

        metric_sample_idxs = np.random.randint(
            0, dataset.imgs.shape[0], n_samples_for_metrics
        )
        self.gt_inception_features = dataset.inception_features[metric_sample_idxs]

    def get_metrics(self, generated_images):
        generated_inception_features = self.inception_feature_extractor.get_features(
            generated_images
        )
    ...
    def update_and_log_metrics(
        ...
        metrics_values_for_txt = np.stack(
            (step_numbers_for_logs, metrics_dict["kid"], metrics_dict["fid"]), axis=1
        )

        np.savetxt(
            os.path.join(output_dir, "inception_metrics.txt"),
