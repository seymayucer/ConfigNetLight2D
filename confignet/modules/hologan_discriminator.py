...
class HologanDiscriminator(tf.keras.models.Model):
    def __init__(
        ...
        img_shape,
        num_resample,
        disc_max_feature_maps,
        disc_kernel_size,
        disc_expansion_factor,
        ...
        self.out_size = (
            int(img_shape[0] / np.power(2, num_resample)),
        ...
        for _ in range(num_resample):
            ...
            conv_block = DiscrBlock(
                num_feature_maps=num_feature_maps,
                kernel_size=disc_kernel_size,
                return_styles=True,
                conv_non_linear=keras.layers.LeakyReLU,
            )
            style_classifier = keras.layers.Dense(
                1, input_shape=(2 * num_feature_maps,)
            )
        ...
        self.num_linear_in = (
            max_feature_maps(e * disc_max_feature_maps // 2)
            * self.out_size[0]
            * self.out_size[1]
        )

        # dense layers standard gan loss (predicting true/false)
        self.disc_map = keras.layers.Dense(
            1, input_shape=(self.num_linear_in,), activation=None, use_bias=True
        )
    ...
    def call(self, input_img):
        ...
        outputs = {}
        for i, (conv_block, style_classifier) in enumerate(
            zip(self.conv_blocks, self.style_classifiers)
        ):
...
class HologanLatentRegressor(tf.keras.models.Model):
    def __init__(
        ...
        self.latent_predictor = keras.layers.Dense(
            latent_dim,
            input_shape=(self.num_linear_in,),
            activation=None,
            use_bias=True,
        )
