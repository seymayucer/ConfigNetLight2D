...
from .ops.upfirdn_2d import upsample_conv_2d
from .tf_utils import get_runtime_coef
...
class modulated_conv2d(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        (
            x_shape,
            w_latents_shape,
        ) = input_shape  # x_shape = [batch_size, height, width, channels]
        ...
        self.fully_connected = fully_connected(
            units=x_shape[-1], apply_lrelu=False, name="modulate"
        )
    ...
    def call(self, inputs, training=None):
        ...
        ww *= tf.cast(
            s[:, tf.newaxis, tf.newaxis, :, tf.newaxis], w.dtype
        ...
        if self.fused_modconv:
            ...
            x *= tf.cast(
                s[:, :, tf.newaxis, tf.newaxis], x.dtype
        ...
        if self.fused_modconv:
            ...
            x *= tf.cast(
                d[:, :, tf.newaxis, tf.newaxis], x.dtype
