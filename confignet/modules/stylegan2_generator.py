...
from .layers.modulated_conv2d import modulated_conv2d
from .layers.fully_connected import fully_connected
from .layers.tf_utils import scaled_lrelu
from .layers.ops.upfirdn_2d import upsample_2d
    ...
    def call(self, inputs, training=None):
        if self.randomize_noise:
            noise = tf.random.normal(
                [tf.shape(inputs)[0], self.noise_shape[1], self.noise_shape[2], 1]
            )
        else:
            noise = self.noise
...
class gen_block(Layer):
    def __init__(self, filters, randomize_noise, up=False, impl="ref", **kwargs):
        ...
        self.modulated_conv2d = modulated_conv2d(
            filters=filters, kernel_size=3, up=up, impl=impl
        )
...
class StyleGAN2Generator(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_layers = tf.cast(
            self.res_log2 * 2 - 2, dtype=tf.int32
        )  # that is how we decide the number of layers
        self.layer_idx = tf.range(self.num_layers)[tf.newaxis, :, tf.newaxis]

        self.mapping = self.g_mapping()
    ...
    def g_mapping(self, name="g_mapping"):
        ...
        latents_out = Lambda(
            lambda x: tf.tile(x[:, tf.newaxis, :], [1, self.num_layers, 1]),
            name="broadcast_latents",
        )(w_latents)

        return Model(inputs=latents_in, outputs=[latents_out, w_latents], name=name,)
    ...
    def g_synthesis(self, randomize_noise, config, impl, name="g_synthesis"):
        ...
        x = gen_block(
            filters=512, randomize_noise=randomize_noise, impl=impl, name="4x4"
        )([constant, w_latents[:, 0]])
        y = modulated_conv2d(
            filters=3, kernel_size=1, demodulate=False, impl=impl, name="4x4_ToRGB"
        )([x, w_latents[:, 1]])
        for index, (res, fmaps) in enumerate(
            list(filters.items())[1 : self.res_log2 - 1]
        ):
            x = gen_block(
                filters=fmaps,
            ...
            y = Lambda(
                lambda x: upsample_2d(x, k=[1, 3, 3, 1], data_format="NHWC", impl=impl),
                name=f"{res}x{res}_img_up",
            )(y)
            y += modulated_conv2d(
                filters=3,
                kernel_size=1,
    ...
    def style_mixing_regularization(self, latents_in, w_latents):
        """ Perform style mixing regularization """
        latents2 = tf.random.normal(tf.shape(latents_in))
        (w_latents2, w) = self.mapping(latents2)
        ...
        return tf.where(
            tf.broadcast_to(self.layer_idx < mixing_cutoff, tf.shape(w_latents)),
            w_latents,
            w_latents2,
        )
    ...
    def call(self, inputs, truncation_psi=0.5, return_latents=False, training=None):
        ...
        (w_latents, the_w) = self.mapping(latents_in)

        if training:
            self.update_moving_average(w_latents)
            w_latents = self.style_mixing_regularization(latents_in, w_latents)
        else:
