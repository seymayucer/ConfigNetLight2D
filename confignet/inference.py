...
from .dataset_utils import imsave, create_dir
...
class Inferencer:
    def __init__(
        ...
        **kwargs,
        ...
        self.Gs = StyleGAN2Generator(resolution, config, randomize_noise=False)
    ...
    def _get_labels(self, label, length):
        if self.num_labels > 0:  # Contain labels
            labels_indice = (
                ...
                else tf.random.uniform([length], 0, self.num_labels, dtype=tf.int32)
            )
    ...
    def genetate_example(self, num_examples, batch_size=1, label=None):
        ...
        for begin in range(0, num_examples, batch_size):
            ...
            images = self.Gs(latents, self.truncation_psi, training=False)
            for idx, (image, indice) in enumerate(zip(images, labels_indice)):
                imsave(
                    image, f"{self.result_path}/example/{begin+idx}_label-{indice}.jpg"
                )
    ...
    def style_mixing_example(self, row_seeds, col_seeds, label=None, col_styles="0-6"):
        ...
        all_z = tf.stack(
            [tf.random.normal([512], seed=seed) for seed in all_seeds]
        )  # [minibatch, component]

        print("Generating images...")
        all_images, all_w = self.Gs(
            all_z, self.truncation_psi, return_latents=True, training=False,
        )
        all_images = (all_images + 1) * 127.5
        all_images = np.clip(all_images, 0.0, 255.0).astype(np.uint8)
        w_dict = {
            seed: w for seed, w in zip(all_seeds, list(all_w))
        }  # [layer, component]
        image_dict = {
        ...
        for (row_seed, col_seed), image in image_dict.items():
            Image.fromarray(image, "RGB").save(
                f"{self.result_path}/mixing/{row_seed}-{col_seed}.jpg"
            )
        ...
        canvas = Image.new(
            "RGB",
            (
                self.resolution * (len(col_seeds) + 1),
                self.resolution * (len(row_seeds) + 1),
            ),
            "black",
        )
        for row_idx, row_seed in enumerate([None] + row_seeds):
            for col_idx, col_seed in enumerate([None] + col_seeds):
                ...
                canvas.paste(
                    Image.fromarray(image_dict[key], "RGB"),
                    (self.resolution * col_idx, self.resolution * row_idx),
                )
    ...
    def generate_gif(
        ...
        for i in range(num_phases):
            ...
            for j in range(transition_frames):
                dlatents = (
                    dlatents0 * (transition_frames - j) + dlatents1 * j
                ) / transition_frames
                output_seq.append(to_image_grid(self.Gs.synthesis(dlatents)))
            output_seq.extend(
                [to_image_grid(self.Gs.synthesis(dlatents1))] * static_frames
            )
